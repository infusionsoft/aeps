...
We are not sure how Thryv currently implements filtering and searching in their APIs.
...
- New DSL to learn.
- We don't know how easy or difficult it will be for Thryv APIs to adopt.
...
|-------------------|-------------------:|----------------:|------------:|---------------:|------------:|-------------:|---------------:|---------------:|
| Current `filter`  |                  3 |               2 |           5 |              3 |           3 |            2 |              4 |              ? |
| OData             |                  2 |               2 |           4 |              2 |           2 |            3 |              3 |              ? |
| RSQL              |                  3 |               2 |           5 |              2 |           3 |            3 |              3 |              ? |
| Google AIP-160    |                  3 |               2 |           5 |              2 |           3 |            4 |              3 |              ? |
| Custom DSL        |                  2 |               2 |           5 |              1 |           3 |            1 |              2 |              ? |
| `field_op=value`  |                  5 |               5 |           5 |              5 |           5 |            4 |              5 |              ? |
| `field[op]=value` |                  4 |               4 |           5 |              4 |           5 |            4 |              4 |              ? |
| `field=op:value`  |                  3 |               3 |           5 |              4 |           4 |            4 |              3 |              ? |
| Query DSL         |                  3 |               3 |           5 |              4 |           4 |            5 |              3 |              ? |
| GraphQL-style     |                  4 |               4 |           5 |              4 |           4 |            5 |              3 |              ? |
| Stay the same     |                  3 |               3 |           5 |              4 |           3 |            1 |              1 |              ? |

| Option            | Score | Notes                                                                                                                         |
|-------------------|-------|-------------------------------------------------------------------------------------------------------------------------------|
| `field_op=value`  | 4.86  | Strongest OpenAPI/SDK/enforcement; cross-field OR usually needs a separate search mechanism.                                  |
| `field[op]=value` | 4.29  | Similar to 2.1; slightly more parsing/encoding quirks.                                                                        |
| GraphQL-style     | 4.14  | Most future-proof for complex queries with good ergonomics; requires a well-defined schema/contract.                          |
| Query DSL         | 3.86  | Most expressive, but higher complexity and easier to overbuild; needs strict guardrails.                                      |
| `field=op:value`  | 3.71  | Good balance, but delimiter/literal edge cases and potential confusion reduce fit slightly.                                   |
| Current `filter`  | 2.86  | Minimal disruption for existing public clients, but weakest on doc/SDK clarity if enforced broadly.                           |
| Google AIP-160    | 2.86  | Standardized and expressive; still single-string filter so doc/SDK clarity and multi-language consistency require extra work. |
| RSQL              | 2.86  | Expressive, but weaker on OpenAPI/SDK clarity and cross-language consistency.                                                 |
| Stay the same     | 2.71  | Pragmatic migration path; governance burden and drift risk due to two syntaxes.                                               |
| OData             | 2.43  | Big surface area; often becomes “support a subset,” plus tooling/library concerns in some stacks.                             |
| Custom DSL        | 2.29  | Highest build/maintenance cost; worst cross-language story without major investment.                                          |
...
The consistent failure of standardized DSL approaches (OData, FIQL, RSQL) to maintain active multi-language ecosystems
suggests fundamental problems with the single-parameter model. While we could attribute each failure to individual
factors, the pattern indicates deeper issues: DSLs require significant ongoing investment in parser maintenance,
struggle to integrate with modern API tooling (OpenAPI, SDK generation), and often end up partially implemented anyway.
