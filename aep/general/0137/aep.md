...
APIs **may** provide an apply action for a resource if it is valuable for
users.
...
**Important:** `Apply` is NOT, and **must not** be used as, an [Update]. Apply
means _replace_: "put _this_ exact representation at _this_ location, replacing
whatever was there before (if anything)". For _modifying_ existing resources,
APIs **must** use [Update] to ensure forward-compatible requests (see
[PATCH and PUT](#patch-and-put)).

### Behavior

The resource **must** be created with the ID specified in the URI, or _not at
all_.
...
**Warning:** This effectively deletes data. If a client fetches a resource,
modifies one field, and applies it back without including the other fields,
those other fields _will be erased_. If you need _updates_ where omitted fields
remain unchanged, see [Update].

### Operation

`Apply` operations are specified using the following pattern:

- The HTTP method **must** be [PUT], and **must** follow the `PUT` guidelines
  in AEP-67.
...
- The response **may** include a [Location] header containing the URI of the
...
- **must** include any server-generated fields (e.g., `createdTime`,
  `updatedTime`).
...
An `Apply` action **must** return appropriate error responses. For additional
