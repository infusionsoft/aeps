...
The `Delete` action **should** succeed even if nothing was deleted. If the resource did
not exist, the action **should not** return [404 Not Found]. See [idempotency](#idempotency).
...
- If a delete request contains a body, the body **must** be ignored and **must not** cause an error (this is required
  by [RFC 9110][])
- The request **must not** _require_ any query parameters.
    - Optional query parameters **may** be included for additional options (e.g., `cascade=true`), but the core deletion
      operation must be determined by the URI alone.
...
- Delete actions **should** return [204 No Content] with no response body if the delete was successful.
    - A response body **may** be included if there is a valid need for additional information.
...
A `Delete` action **must** return appropriate error responses. For additional guidance, see [Errors]
...
**should** provide a `bool` `cascade` field on the request, which the user sets to
...
`Delete` operations **must** be idempotent. When a client attempts to delete a resource that has already been deleted or
does not exist, the API **must** treat the operation as successful and return the same response as if the resource was
just deleted.

This treats deletion as "ensure this resource does not exist" rather than "remove this specific resource." The approach
simplifies client retry logic and aligns with idempotency principles: repeated identical requests produce the same
result.
...
However, APIs **may** return [200 OK] with a response body when additional information is useful:
...
* **2026-02-20:** Initial creation, adapted from [Google AIP-135][] and aep.dev [AEP-135][].
